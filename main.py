...
def add_page_number(canvas,_,team):
    ...
    width, _ = letter
    canvas.drawString(width, 50, team)
    ...
    if uploaded_files:
        ...
        if len(uploaded_dfs) > 1:
            ...
            for df in uploaded_dfs[1:]:
                merged_df = pd.merge(merged_df, df, on=merged_df.columns[0], how="inner", validate="1:1").astype(str)

            merged_df = merged_df.replace('nan', '')
            merged_df.iloc[:, 0] = merged_df.iloc[:, 0].astype(str).str.replace(r'\.0$', '', regex=True)
            merged_df.iloc[:, 1] = merged_df.iloc[:, 1].astype(str).str.replace(r'\.0$', '', regex=True)
            ...
            if st.button("Generate PDF"):
                ...
                for row_page in range(num_row_pages):
                    ...
                    for col_page in range(num_col_pages):
                        ...
                        style_n.fontSize = fontsize
                        style_n.leading = 8

                        # Wrap table data into Paragraphs
                        wrapped_data = [[Paragraph(str(item), style_n) for item in row] for row in page_data]

                        # Create table and apply styles
                        table = Table(wrapped_data, colWidths=col_widths[start_col:end_col], rowHeights=37)
                        ...
                        while col_idx < len(page_data[0]):
                            ...
                            if start_idx != col_idx:
                                ...
                                for i in range(start_idx + 1, col_idx + 1):
                                    wrapped_data[0][i] = Paragraph("", style_n)
